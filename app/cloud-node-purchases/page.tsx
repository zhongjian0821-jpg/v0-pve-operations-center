...
export default function CloudNodePurchasesPage() {
  ...
  useEffect(() => {
    fetch('/api/admin/cloud-node-purchases').then(res => res.json()).then(data => {
  ...
  return (
    <div className="p-8">
      <h1 className="text-3xl font-bold mb-6">云节点购买记录</h1>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50"><tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">总价</th>
            </tr></thead>
              ...
              {purchases.length === 0 ? <tr><td colSpan={5} className="px-6 py-4 text-center text-gray-500">暂无数据</td></tr> :
                purchases.map(p => (
                  <tr key={p.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm">{p.node_type}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm">{p.quantity}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-bold">${p.total_price}</td>
                  </tr>
                ))
              }
            </tbody>
