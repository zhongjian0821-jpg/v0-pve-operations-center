...
export default function CloudNodePurchasesPage() {
  ...
  const fetchPurchases = async () => {
    try {
      const token = localStorage.getItem('admin_token');
      const response = await fetch('/api/admin/cloud-node-purchases', {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const token = localStorage.getItem('admin_token');
      const response = await fetch(`/api/admin/cloud-node-purchases?id=${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      const data = await response.json();
  ...
  return (
    <div className="p-8">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold">云节点购买管理</h1>
          <p className="text-gray-500 mt-1">管理云节点购买记录</p>
        </div>
