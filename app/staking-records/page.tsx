...
export default function StakingrecordsPage() {
  ...
  const fetchRecords = async () => {
    try {
      ...
      const response = await fetch('/api/admin/staking-records', {
        headers: {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const token = localStorage.getItem('admin_token');
      const response = await fetch(`/api/admin/staking-records?id=${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <h1 className="text-2xl font-bold text-gray-900">质押记录</h1>
            </div>
