  ...
  try {
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        totalNodes: parseInt(stats[1].rows[0]?.total || '0'),
        totalOrders: parseInt(stats[2].rows[0]?.total || '0'),
        totalWithdrawals: parseFloat(stats[3].rows[0]?.total || '0'),
        totalAshvaBalance: parseFloat(stats[4].rows[0]?.total || '0'),
      }
    });
  } catch (error: any) {
