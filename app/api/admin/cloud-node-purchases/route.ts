...
export async function GET(request: NextRequest) {
  try {
    ...
    if (id) {
      const record = await sql`SELECT * FROM cloud_node_purchases WHERE id = ${id}`;
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await sql`
      INSERT INTO cloud_node_purchases DEFAULT VALUES RETURNING *
    `;
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const result = await sql`DELETE FROM cloud_node_purchases WHERE id = ${id} RETURNING *`;
    if (result.length === 0) return errorResponse('Record not found', 404);
