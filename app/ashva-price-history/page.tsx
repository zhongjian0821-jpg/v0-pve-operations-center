...
export default function AshvapricehistoryPage() {
  ...
  const fetchRecords = async () => {
    try {
      ...
      const response = await fetch('/api/admin/ashva-price-history', {
        headers: {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const token = localStorage.getItem('admin_token');
      const response = await fetch(`/api/admin/ashva-price-history?id=${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <h1 className="text-2xl font-bold text-gray-900">ASHVA价格历史</h1>
            </div>
